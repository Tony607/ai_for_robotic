...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # Add your code here!
    #
    #set the dimension of the matrix and the vector
    dim = 2 * (N + num_landmarks)# multipied by 2 because x and y
    Omega = matrix()
    Omega.zero(dim, dim)
    #constrains of the initial position
    Omega.value[0][0] = 1.0
    Omega.value[1][1] = 1.0
    Xi = matrix()
    Xi.zero(dim, 1)
    ...
    for k in range(len(data)):
        ...
        #might be multiple landmarks measurements
        for i in range(len(measurement)):
            # m is the index of the landmark coordinate in the matrix/vector
            #landmarks cordinates start from 2*N
            m = 2 * (N + measurement[i][0])
            #update the information matrix/vector based on the measurement(of landmark)
            for b in range(2):#for x and y
                #Omg[Xk][Xk]  Xk means location k(k->0~N-1)
                Omega.value[n+b][n+b] +=  1.0 / measurement_noise
                #Omg[Li][Li]  Li means landmark i(i->0~num_landmarks-1)
